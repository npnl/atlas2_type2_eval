...
class GrandChallengesLoader():
    ...
    def __init__(self, loader_settings, prediction_json_filepath='/input/predictions.json'):
        ...
        # inp_path = self.json_data['ground_truth_path']
        # self.loader_settings['InputPath'] = self.json_data['ground_truth_path']
    ...
    def __len__(self):
        return len(self.prediction_paths)
    ...
    def mapping(self):
        ...
        for entity in self.json_data:
            ent_inputs = entity['inputs']  # Specifies the input that was sent to the model
            ent_outputs = entity['outputs']  # Specifies the output of the model
            job_pk = entity['pk']
            ground_truth_root = self.loader_settings["GroundTruthPath"]
            # We need to get the original name of the model from the output of the model
            for channel_data in ent_inputs:  # "inputs" is a list
                original_name = channel_data["image"]["name"]
                ground_truth_filepath = os.path.join(ground_truth_root, original_name)
                break

            for channel_data in ent_outputs:
                prediction_name = channel_data["image"]["pk"]
                if(not prediction_name.endswith('.mha')):
                    prediction_name += '.mha'
                prediction_relative_path = channel_data['interface']['relative_path']
                break
            prediction_filepath = f'/input/{job_pk}/output/{prediction_relative_path}/{prediction_name}'

            self.prediction_paths.append(prediction_filepath)
            self.ground_truth_paths.append(ground_truth_filepath)
        ...
        #     if(pred is not None):
        #         expected_prediction = f'/input/{pred}/{self.loader_settings["OutputPath"]}'
    ...
    def load_list(self, filepath_list: list) -> np.array:
        ...
        if(self.image_dimensions is None):
            # Image dimensions unknown; load one and set it
            im, _ = medpy.io.load(filepath_list[0])
            self.image_dimensions = im.shape
    ...
    def load_eval_batches(self):
        ...
        for idx in range(0, num_samples, self.batch_size):
            batch_preds = self.prediction_paths[idx:idx+self.batch_size]
            batch_ground_truth = self.ground_truth_paths[idx:idx+self.batch_size]
            print(f'loading preds: {batch_preds}')
            print(f'loading truth: {batch_ground_truth}')
            pred_data = self.load_list(batch_preds)
            truth_data = self.load_list(batch_ground_truth)
